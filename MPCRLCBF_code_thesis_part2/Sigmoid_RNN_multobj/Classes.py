...
class Obstacles:
    ...
    def h_obsfunc(self, x):
        """
        resturns list of CBF funcs ( h(x) ) but in a numerical format
        """
        h_list = []
        for (cx, cy), r in zip(self.positions, self.radii):
            h_list.append((x[0] - cx)**2 + (x[1] - cy)**2 - r**2)
        return h_list
    ...
    def make_h_functions(self):
        ...
        """
        funcs = []
        # create one symbolic 2-vector “x” that each h_i will share
        x = cs.MX.sym("x", 4)

        for idx, ((cx, cy), r) in enumerate(zip(self.positions, self.radii)):
            # build the i-th obstacle expression
            hi_expr = (x[0] - cx)**2 + (x[1] - cy)**2 - r**2
            # make a casadi.Function named “h{i}” that maps x → hi_expr
            hi_fun = cs.Function(f"h{idx+1}", [x], [hi_expr])
            funcs.append(hi_fun)
...
class RNN:
    ...
    def forward_rnn(self):
        ...
        for i in range(self.horizon):
            ...
            *h, y = rnn_step(*h, self.input_sym[:,i], *params)
    ...
    def initialize_parameters(self):
        ...
        for i in range(L):
            ...
            bound1 = np.sqrt(6.0 / (fan_in + fan_out))
            Wih_v = self.np_random.uniform(-bound1, bound1, size=(fan_out, fan_in))
            ...
            if i < L-1:
                bound2 = np.sqrt(6.0 / (fan_out + fan_out))
                Whh_v = self.np_random.uniform(-bound2, bound2, size=(fan_out, fan_out))
...
class MPC:
    ...
    def __init__(self, layers_list, horizon, positions, radii):
        ...
        flat_input = flat_input_fn(self.X_sym)
        ...
        # 4) Pick whichever output you want as alpha_list
    ...
    def make_flat_input_fn(self):
        """
        Returns a CasADi Function flat_in(X) → flat_input_vector,
        where X is ns×horizon+1) and flat_input_vector is
        [ x₀; h₁(x₀); …; hₘ(x₀); x₁; h₁(x₁); …; hₘ(x₁); …; x_{h-1}; h₁(x_{h-1}); …; hₘ(x_{h-1}) ].
        """
        ...
        for t in range(self.horizon):
            x_t    = X[:, t]
            cbf_t  = [h_i(x_t) for h_i in self.h_funcs]  # m×1 each
            inter.append(x_t)                            # ns×1
            inter.extend(cbf_t)                          # m scalars

        flat_in = cs.vertcat(*inter)  # ((ns+m)*horizon)×1
        return cs.Function("flat_input", [X], [flat_in], ["X"], ["flat_in"])
    ...
    def MPC_solver_noslack(self):
        ...
        nlp = {
            ...
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.rnn.get_flat_parameters(), *self.hid_syms),
    ...
    def MPC_solver_rand(self):
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, S_flat),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.rnn.get_flat_parameters(), rand_noise, *self.hid_syms),
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        # X_con + U_con + S_con 
        lagrange_mult_x_lb_sym = cs.MX.sym("lagrange_mult_x_lb_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + self.rnn.obst.obstacle_num * (self.horizon))
        lagrange_mult_x_ub_sym = cs.MX.sym("lagrange_mult_x_ub_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + self.rnn.obst.obstacle_num  * (self.horizon))
        lagrange_mult_g_sym = cs.MX.sym("lagrange_mult_g_sym", 1*self.ns*(self.horizon) + self.rnn.obst.obstacle_num*self.horizon)
        ...
        lbx = cs.vertcat(self.X_sym[:,0], cs.DM(X_lower_bound), self.U_sym[:,0], cs.DM(U_lower_bound), np.zeros(self.rnn.obst.obstacle_num *self.horizon)) 
        ubx = cs.vertcat(self.X_sym[:,0], cs.DM(X_upper_bound), self.U_sym[:,0], cs.DM(U_upper_bound), np.inf*np.ones(self.rnn.obst.obstacle_num *self.horizon))
...
class RLclass:
        ...
        def plot_B_update(self, B_update_history, experiment_folder):
            ...
            rnn_B_update = B_update[:, 4:]
            # take mean across rows (7,205) --> (7,)
            mean_mag = np.mean(np.abs(rnn_B_update), axis=1)
            ...
            # 2. Plot just the rnn mean
            fig_rnn = plt.figure()
            plt.plot(mean_mag, "o-", label='mean |rnn_B_update|')
            plt.xlabel('Update iteration')
            ...
            plt.grid(True)
            plt.tight_layout()
            self.save_figures([(fig_rnn, 'rnn_mean_B_update_over_time')], experiment_folder)
            plt.close(fig_rnn)
        ...
        def V_MPC(self, params, x, hidden_in):
            # bounds
            ...
            solution = self.solver_inst(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat,  params["rnn_params"], *hidden_in),
                ubx=ubx,  
                lbx=lbx,
                ubg=ubg,
                lbg=lbg
            ...
            flat_input = self.flat_input_fn(X)
        ...
        def V_MPC_rand(self, params, x, rand, hidden_in):
            """
            ...
            solution = self.solver_inst_random(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat, params["rnn_params"], rand, *hidden_in),
                ubx=ubx,  
                lbx=lbx,
                ubg=ubg,
                lbg=lbg
        ...
        def Q_MPC(self, params, action, x, hidden_in):
            ...
            lbx = np.concatenate([np.asarray(x).flatten(), self.X_lower_bound, np.asarray(action).flatten(), U_lower_bound,  np.zeros(self.mpc.rnn.obst.obstacle_num *self.horizon)])  
            ubx = np.concatenate([np.asarray(x).flatten(), self.X_upper_bound, np.asarray(action).flatten(), U_upper_bound, np.inf*np.ones(self.mpc.rnn.obst.obstacle_num *self.horizon)])
        ...
        def evaluation_step(self, S, params, experiment_folder, episode_duration):
                ...
                for i in range(episode_duration):
                    action, _, hidden_in_VMPC = self.V_MPC(params=params, x=state, hidden_in=hidden_in_VMPC)
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, replay_buffer, episode_updatefreq, experiment_folder):
            ...
            for i in range(1,episode_duration*num_episodes):  
                ...
                u, hidden_in_VMPCrand = self.V_MPC_rand(params=params, x=x, rand = rand, hidden_in=hidden_in_VMPCrand)
                ...
                solution, Qcost, lagrange_mult_g, lam_lbx, lam_ubx, _, hidden_in_QMPC = self.Q_MPC(params=params, action=u, x=x, hidden_in=hidden_in_QMPC)
                ...
                _, Vcost, hidden_in_VMPC = self.V_MPC(params=params, x=x, hidden_in=hidden_in_VMPC)
                ...
                qlagrange_numeric_jacob = self.qlagrange_fn_jacob(
                    ...
                    params["R"],               # self.R_sym
                    params["P"],               # self.P_sym
                    lam_lbx,                   # lagrange_mult_x_lb_sym
                    lam_ubx,                   # lagrange_mult_x_ub_sym
                    lagrange_mult_g,           # lagrange_mult_g_sym
                    X,                         # X_flat
                    U,                         # U_flat
                    S,                         # S_flat
                    params["rnn_params"],      # your flattened rnn parameters
                    *hidden_in_QMPC            # your hidden‐state inputs
                )
                ...
                if (k == episode_duration):                     
                    ...
                    if (current_episode % 20) == 0:
                        ...
                        rnn_grads = gradst[:, 4:]
                        # take mean across rows (7,205) --> (7,)
                        mean_mag = np.mean(np.abs(rnn_grads), axis=1)
                        ...
                        figures_training = [
                            ...
                            (rnn_figgrad, f"rnn_grad_plotat_{i}"),
                            ]
