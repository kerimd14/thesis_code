...
def plot_stagecost_compare_validation(nn_file, rnn_file):
    """
    Compare 'stage_cost_valid' from two NPZ files (NN vs RNN) and plot them.
    Saves plot as 'stage_cost_valid_comparison.svg' in the same folder as nn_file.

    Args:
        nn_file : str
            Path to the NPZ file for the NN experiment.
        rnn_file : str
            Path to the NPZ file for the RNN experiment.
    """
    ...
    plt.figure()
    plt.plot(x_nn, vals_nn, "o-", label="NN")
    plt.plot(x_rnn, vals_rnn, "o-", label="RNN")
    plt.xlabel("Evaluation #")
    plt.ylabel("Sum of stage cost")
    plt.title("Validation Stage Cost Comparison")
    plt.grid(True)
    plt.legend()

    # Save in same directory as nn_file
    out_dir = os.path.dirname(nn_file)
    out_path = os.path.join(out_dir, "stage_cost_valid_comparison.svg")
    plt.tight_layout()
...
def plot_simulation_npz(npz_path: str | Path, out_root: str | Path = "thesis_plots_2",
    ...
    montage_path = svg_dir / f"NN_snapshots_{suffix}.svg"
...
def plot_simulation_before(path_or_dir: str | Path, out_root: str | Path = "thesis_plots_2",
                           **kwargs) -> None:
...
def plot_simulation_after(path_or_dir: str | Path, out_root: str | Path = "thesis_plots_2",
                          **kwargs) -> None:
    p = Path(path_or_dir).expanduser().resolve()
...
if __name__ == "__main__":
    ...
    nn_file_before_sim = r"C:\Users\kerim\OneDrive\Documents\Thesis_CODE\MPCRLCBF_code_thesis_part2\Sigmoid_NN_mult_move_obj\NNSigmoid_41_noP_diag_stagecost_54554049.82\thesis_data_nn\simulation_before.npz"
    nn_file_after_sim = r"C:\Users\kerim\OneDrive\Documents\Thesis_CODE\MPCRLCBF_code_thesis_part2\Sigmoid_NN_mult_move_obj\NNSigmoid_41_noP_diag_stagecost_54554049.82\thesis_data_nn\simulation_after.npz"

    plot_simulation_before(nn_file_before_sim, out_root="thesis_plots_2")
    plot_simulation_after(nn_file_after_sim,  out_root="thesis_plots_2")
