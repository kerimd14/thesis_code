...
class MPC:
    ...
    def cbf_const(self):
        ...
        for k in range(self.horizon):
            ...
            alpha_list = self.alpha_nn(cs.vertcat(xk, *nn_in_list))
...
class RLclass:
        ...
        def evaluation_step(self, params, experiment_folder, episode_duration):
                """
                Run one evaluation episode using the current policy (V-MPC).

                Args:
                    params (dict):
                        Current system & RNN parameters.
                    experiment_folder (str):
                        Base folder where evaluation figures & GIF are saved.
                    episode_duration (int):
                        Number of time steps in the episode.

                Returns:
                    None.  
                    Saves trajectory plots, action/time-series, cost/time-series, velocity/time-series,
                    and a system+obstacle animation to disk, and decays the learning rate if needed.
                """
                ...
                #Pre-compute initial obstacle predictions and initial RNN hidden state
