...
def plot_simulation_npz(npz_path: str | Path, out_root: str | Path = "thesis_plots",
    ...
    make_system_obstacle_montage_v1(
        ...
        k_fontsize=20, figsize_per_ax=(5.0, 5.0),
    ...
    if alphas is not None:
        ...
        A = np.squeeze(alphas)
        if A.ndim == 1:
            plt.plot(A, "o-", label=r"$\alpha(x_k)$")
        else:
            for i in range(A.shape[1]):
                plt.plot(A[:, i], "o-", label=rf"$\alpha_{{{i+1}}}(x_k)$")
        plt.xlabel(r"Iteration $k$"); plt.ylabel(r"$\alpha_i(x_k)$")
    ...
    if hx is not None:
        ...
        for i in range(hx.shape[1]):
            ...
            plt.scatter(np.arange(N), hx[:, i], c=np.arange(N), cmap=cmap, norm=norm, s=20)
            plt.xlabel(r"Iteration $k$"); plt.ylabel(rf"$h_{{{i+1}}}(x_k)$")
    ...
    if slacks_eval is not None:
        ...
        for oi in range(m):
            ...
            plt.legend(ncol=min(4, Nh), fontsize="small")
            plt.tight_layout()
...
if __name__ == "__main__":
    ...
    nn_file_before_sim = r"C:\Users\kerim\OneDrive\Documents\Thesis_CODE\MPCRLCBF_code_thesis_part2\Sigmoid_NN_mult_move_obj\NNSigmoid_35_stagecost_98238416.40\thesis_data_nn\simulation_before.npz"
    nn_file_after_sim = r"C:\Users\kerim\OneDrive\Documents\Thesis_CODE\MPCRLCBF_code_thesis_part2\Sigmoid_NN_mult_move_obj\NNSigmoid_35_stagecost_98238416.40\thesis_data_nn\simulation_after.npz"
