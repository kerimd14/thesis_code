...
def flat_input_fn(mpc, X, U, horizon, xpred_hor, ypred_hor, m):
    X = cs.reshape(X, mpc.ns, mpc.horizon + 1)
    U = cs.reshape(U, mpc.na, mpc.horizon)
    inter = []
...
def MPC_func(x, mpc, params, solver_inst, xpred_list, ypred_list, hidden_in, m, x_prev, lam_x_prev, lam_g_prev, layers_list):
        ...
        U = solution["x"][mpc.ns * (mpc.horizon+1):mpc.ns * (mpc.horizon+1) + mpc.na * mpc.horizon]
        flat_input = flat_input_fn(mpc, X, U,
