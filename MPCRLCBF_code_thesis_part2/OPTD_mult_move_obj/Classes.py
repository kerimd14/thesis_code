...
class MPC:
    ...
    def __init__(self, layers_list, horizon, positions, radii):
        ...
        self.OMEGAS_sym = cs.MX.sym("omega", self.horizon, self.m) #for horizon and for each obstacle
        ...
        self.Pw_sym = cs.MX.sym("Pw", self.horizon, self.m)
        self.OMEGAS0_sym = cs.MX.sym("omega0", self.horizon, self.m)
    ...
    def cbf_const(self):
        ...
        for k in range(self.horizon):
            ...
            for i, h_i in enumerate(self.h_funcs):
                ...
                h_xnext = h_i(x_next, self.xpred_hor[k*self.m:(k+1)*self.m], self.ypred_hor[k*self.m:(k+1)*self.m])                      # h(x_{k+1})

                # φ_i = h(x_{k+1}) − h(x_k) + α⋅h(x_k)
                phi_i = h_xnext - h_x + omega_ki * h_x
    ...
    def objective_method(self):
        ...
        stage_cost = sum(
            ...
            self.weight_cbf * self.S_sym[m,k] + self.Pw_sym[k,m]*((self.OMEGAS_sym[k,m] - self.OMEGAS0_sym[k,m])**2))
    ...
    def objective_method_noslack(self):
        ...
        stage_cost = sum(
            (self.X_sym[:, k].T @ self.Q_sym @ self.X_sym[:, k] + 
            self.U_sym[:, k].T @ self.R_sym @ self.U_sym[:, k] + self.Pw_sym[k,m]*((self.OMEGAS_sym[k,m] - self.OMEGAS0_sym[k,m])**2))
            for m in range (self.m) for k in range(self.horizon)
        )
        #slack penalty
        terminal_cost = cs.bilin((self.P_sym), self.X_sym[:, -1])
    ...
    def MPC_solver_noslack(self):
        ...
        OMEGAS_sym_flat = cs.reshape(self.OMEGAS_sym , -1, 1)
        Pw_sym_flat = cs.reshape(self.Pw_sym , -1, 1)
        OMEGAS0_sym_flat = cs.reshape(self.OMEGAS0_sym , -1, 1)

        nlp = {
            "x": cs.vertcat(X_flat, U_flat, OMEGAS_sym_flat),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, 
                            R_sym_flat, Pw_sym_flat, OMEGAS0_sym_flat, self.xpred_hor, self.ypred_hor),
    ...
    def MPC_solver(self):
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, S_flat, OMEGAS_sym_flat),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, 
                            R_sym_flat, Pw_sym_flat, OMEGAS0_sym_flat, self.xpred_hor, self.ypred_hor),
            "f": self.objective, 
    ...
    def MPC_solver_rand(self):
        ...
        nlp = {
            ...
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, 
                            self.nn.get_flat_parameters(), rand_noise, Pw_sym_flat, OMEGAS0_sym_flat, self.xpred_hor, self.ypred_hor),
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        opt_solution = cs.vertcat(X_flat, U_flat, S_flat, OMEGAS_sym_flat)

      
        # X_con + U_con + S_con 
        lagrange_mult_x_lb_sym = cs.MX.sym("lagrange_mult_x_lb_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + self.nn.obst.obstacle_num * (self.horizon) + self.nn.obst.obstacle_num * (self.horizon))
        lagrange_mult_x_ub_sym = cs.MX.sym("lagrange_mult_x_ub_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + self.nn.obst.obstacle_num  * (self.horizon) + self.nn.obst.obstacle_num * (self.horizon))
        ...
        qlagrange_fn = cs.Function(
            ...
            [
                ...
                lagrange_mult_g_sym, X_flat, U_flat, S_flat, OMEGAS0_sym_flat, Pw_sym_flat,
                self.xpred_hor, self.ypred_hor
            ],
            [qlagrange_sens],
            [
                'A_sym', 'B_sym', 'b_sym', 'Q_sym', 'R_sym','P_sym', 'lagrange_mult_x_lb_sym', 
                'lagrange_mult_x_ub_sym', 'lagrange_mult_g_sym', 'X', 'U', 'S', 'omega0', 'Pw',
                'xpred_hor', 'ypred_hor'
            ],
            ['qlagrange_sens']
        )
