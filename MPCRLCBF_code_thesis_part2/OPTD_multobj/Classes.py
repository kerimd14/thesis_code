...
class NN:
    ...
    def normalization_z(self, nn_input):
        ...
        x_norm = nn_input[:4]#(nn_input[:4] - mu_states) / sigma_states
...
class MPC:
    ...
    def cbf_func(self):
        ...
        phi_list = []
        nn_in_list = [h_i(self.x_sym) for h_i in h_funcs ]
        ...
        return cs.Function(
            "cbff",
            [self.x_sym, self.u_sym, self.nn.get_flat_parameters()],
            [phi_vec],
            ["x", "u", "alpha_params"],
            ["phi_vec"],
            )
    ...
    def cbf_const(self):
        ...
        for k in range(self.horizon):
            xk      = self.X_sym[:, k]        # 4×1
            uk      = self.U_sym[:, k]        # 2×1
            phi_k   = cbf_fn(xk, uk, self.nn.get_flat_parameters())  # m×1
