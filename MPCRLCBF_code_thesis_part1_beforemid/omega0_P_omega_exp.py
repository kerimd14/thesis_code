...
def MPC_func(params, x, mpc, Pw, omega0):
    dt = 0.2
    A = np.array([
        [1, 0, dt, 0],
        [0, 1, 0, dt],
    ...
    lbx = np.concatenate([np.array(x).flatten(), X_lower_bound, U_lower_bound, np.ones(mpc.horizon)* np.array([1e-6])])
    ubx = np.concatenate([np.array(x).flatten(), X_upper_bound, U_upper_bound, np.ones(mpc.horizon)* np.array([1])])
...
def run_simulation_with_P_and_Pw_and_omega0(Pw_list, omega0_list, env_class):
    ...
    for omega0_idx, omega0 in enumerate(omega0_list):
        for pw_idx, Pw in enumerate(Pw_list):
            stage_cost_lst = []
            state, _ = env_instance.reset(seed=42, options={})
            states = [state[:2]]
            mpc = MPC(0.2)  # now P is fixed

            for _ in range(200):
                action, _ = MPC_func(params, state, mpc, Pw, omega0)
                action = cs.fmin(cs.fmax(cs.DM(action), -1), 1)
                state, _, _, _, _ = env_instance.step(action)
                states.append(state[:2])
                stage_cost_lst.append(stage_cost_func(action, state))

            states = np.array(states)
            stage_cost_lst = np.array(stage_cost_lst)

            plt.plot(
                states[:, 0], states[:, 1],
                linestyle='-', marker=marker_list[pw_idx],
                label=f"ω₀: {float(omega0)}, Pw: {float(Pw)}"
            )
            print(f"Trajectory length for ω₀={float(omega0)} and Pw={float(Pw)}:", 
                  calculate_trajectory_length(states))
            print(f"Stage cost for ω₀={float(omega0)} and Pw={float(Pw)}:", 
                  np.sum(stage_cost_lst))
    ...
    plt.xlabel("$x$ (m)")
    plt.ylabel("$y$ (m)")
    plt.title("Trajectories for fixed terminal cost P and varying ω₀, Pw")
    plt.legend()
    plt.axis("equal")
    plt.grid()
    plt.show()
...
# Now create a list of ω₀ values to sweep
omega0_list = [
    cs.DM(0.5),
]

run_simulation_with_P_and_Pw_and_omega0(Pw_list, omega0_list, env)