...
class MPC:
    ...
    def __init__(self, dt):
        ...
        self.weight_sx = cs.DM([1e8, 1e8, 1e8, 1e8])
        ...
        self.Sx_sym = cs.MX.sym("Sx", self.ns, self.horizon)
    ...
    def slack_bx_const(self):

        """""
        used to construct x constraints with slack variables 
        """
        ...
        for k in range(self.horizon):
            ...
            slack_bx_const_list.append(-self.X_sym[:,k+1] - self.Sx_sym[:,k] - np.ones(self.ns) * 5)
            slack_bx_const_list.append(self.X_sym[:,k+1]  - self.Sx_sym[:,k] - np.ones(self.ns) * 5)

        self.slack_bx_const_list = cs.vertcat( *slack_bx_const_list )
    ...
    def objective_method(self):
        ...
        stage_cost = sum(
            ...
            self.weight_cbf * self.S_sym[:,k] + self.weight_sx.T @ self.Sx_sym[:,k])
            for k in range(self.horizon)
        )
        #slack penalty
    ...
    def MPC_solver(self):
        ...
        Sx_flat = cs.reshape(self.Sx_sym, -1, 1)   
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, S_flat, Sx_flat, self.omega_sym),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.Pw_sym, self.omega0_sym),
            "f": self.objective, 
            "g": cs.vertcat(self.state_const_list, -self.cbf_const_list, self.slack_bx_const_list),
        }
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        opt_solution = cs.vertcat(X_flat, U_flat, S_flat, Sx_flat, self.omega_sym)

        # +1 for the omega constraints
        lagrange_mult_x_lb_sym = cs.MX.sym("lagrange_mult_x_lb_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + 1 * (self.horizon) + self.ns * (self.horizon) + 1 * (self.horizon))
        lagrange_mult_x_ub_sym = cs.MX.sym("lagrange_mult_x_ub_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + 1 * (self.horizon) + self.ns * (self.horizon) + 1 * (self.horizon))
        lagrange_mult_g_sym = cs.MX.sym("lagrange_mult_g_sym", 1*self.ns*(self.horizon) + self.horizon + 2*self.ns*(self.horizon))

        # construct 
        X_lower_bound = -5 * np.array([1e2, 1e2, 1, 1])#-5* np.ones(self.ns * (self.horizon))
        X_upper_bound = 5 * np.array([1e2, 1e2, 1, 1])#5* np.ones(self.ns * (self.horizon)) 

        U_lower_bound = -np.ones(self.na * (self.horizon-1))
        U_upper_bound = np.ones(self.na * (self.horizon-1))  


        lbx = cs.vertcat(self.X_sym[:,0], cs.DM(X_lower_bound), self.U_sym[:,0], cs.DM(U_lower_bound), np.zeros(self.horizon), np.zeros(self.horizon*self.ns), 1e-6) 
        ubx = cs.vertcat(self.X_sym[:,0], cs.DM(X_upper_bound), self.U_sym[:,0], cs.DM(U_upper_bound), np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns), 1)
        ...
        lagrange3 = lagrange_mult_g_sym.T @ cs.vertcat(self.state_const_list, self.cbf_const_list, self.slack_bx_const_list) # opposite signs
        ...
        qlagrange_fn = cs.Function(
            ...
            [
                ...
                lagrange_mult_g_sym, X_flat, U_flat, S_flat, Sx_flat, self.omega_sym
            ],
            [qlagrange_sens],
            [
                'A_sym', 'B_sym', 'b_sym', 'Q_sym', 'R_sym',
                'V_sym', 'P_sym', 'Pw_sym', 'omega0_sym', 'lagrange_mult_x_lb_sym', 
                'lagrange_mult_x_ub_sym', 'lagrange_mult_g_sym', 'X', 'U', 'S', 'Sx', 'omega_sym'
            ],
            ['qlagrange_sens']
        )
...
class RLclass:
        ...
        def __init__(self, params_innit, seed, alpha, sampling_time, gamma, decay_rate, noise_scalingfactor, noise_variance):
            ...
            # self.X_lower_bound = -1e6 * 5 * np.ones(self.ns * (self.horizon))
            # self.X_upper_bound = 1e6 * 5 * np.ones(self.ns  * (self.horizon))
            self.X_lower_bound = -5 * np.array([1e2, 1e2, 1, 1])#np.concatenate(-1e1 *5 * np.ones((self.ns-2) * (self.horizon)),-5 * np.ones((self.ns-2) * (self.horizon)))
            self.X_upper_bound = 5 * np.array([1e2, 1e2, 1, 1]) #cs.vertcat(1e1 *5 * np.ones((self.ns-2) * (self.horizon)),5 * np.ones((self.ns-2) * (self.horizon)))
            ...
            self.slack_bx_const_lbg = -np.inf * np.ones(2 * self.ns * (self.horizon))
            self.slack_bx_const_ubg = np.zeros(2 * self.ns * (self.horizon))
        ...
        def V_MPC(self, params, x):
            ...
            lbx = np.concatenate([np.array(x).flatten(), self.X_lower_bound, U_lower_bound,  np.zeros(self.horizon), np.zeros(self.horizon*self.ns), np.array([1e-6])])  
            ubx = np.concatenate([np.array(x).flatten(), self.X_upper_bound, U_upper_bound,  np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns), np.array([1])])

            #lower and upper bound for state and cbf constraints 
            lbg = np.concatenate([self.state_const_lbg, self.cbf_const_lbg, self.slack_bx_const_lbg])  
            ubg = np.concatenate([self.state_const_ubg, self.cbf_const_ubg, self.slack_bx_const_ubg])
        ...
        def Q_MPC(self, params, action, x):
            ...
            lbx = np.concatenate([np.asarray(x).flatten(), self.X_lower_bound, np.asarray(action).flatten(), U_lower_bound, np.zeros(self.horizon), np.zeros(self.horizon*self.ns),  np.array([1e-6])])  
            ubx = np.concatenate([np.asarray(x).flatten(), self.X_upper_bound, np.asarray(action).flatten(), U_upper_bound, np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns), np.array([1])])
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, experiment_folder):
            ...
            for i in range(1,episode_duration*num_episodes):
                ...
                Sx = solution[self.na * (self.horizon) + self.ns * (self.horizon+1) +self.horizon: self.na * (self.horizon) + self.ns * (self.horizon+1) +self.horizon + self.ns * (self.horizon)]
                omega = solution[-1]
                ...
                qlagrange_numeric_jacob=  self.qlagrange_fn_jacob(
                    ...
                    X=X, U=U, S=S, Sx = Sx, omega_sym=omega
                )['qlagrange_sens']
                ...
                if (k == episode_duration): #or ((1e-2 > np.abs(x[0])) and (1e-2 > np.abs(x[1]))):
                    ...
                    # self.noise_variance = self.noise_variance*(1-0.008)
...
def MPC_func(x, mpc, params):
        ...
        # bounds
        X_lower_bound = -5 * np.array([1e2, 1e2, 1, 1])#cs.vertcat(-1e1 *5 * np.ones((mpc.ns-2) * (mpc.horizon)),-5 * np.ones((mpc.ns-2) * (mpc.horizon)))
        X_upper_bound = 5 * np.array([1e2, 1e2, 1, 1])#cs.vertcat(1e1 *5 * np.ones((mpc.ns-2) * (mpc.horizon)),5 * np.ones((mpc.ns-2) * (mpc.horizon)))
        ...
        slack_bx_const_lbg = -np.inf * np.ones(2 * mpc.ns *(mpc.horizon))
        slack_bx_const_ubg = np.zeros(2 * mpc.ns *(mpc.horizon))

        lbx = np.concatenate([np.array(x).flatten(), X_lower_bound, U_lower_bound,   np.zeros(mpc.horizon), np.zeros(mpc.horizon*mpc.ns), np.array([1e-6])])  
        ubx = np.concatenate([np.array(x).flatten(), X_upper_bound, U_upper_bound,  np.inf*np.ones(mpc.horizon), np.inf*np.ones(mpc.horizon* mpc.ns), np.array([1])])

        lbg = np.concatenate([state_const_lbg, cbf_const_lbg, slack_bx_const_lbg])  
        ubg = np.concatenate([state_const_ubg, cbf_const_ubg, slack_bx_const_ubg])
...
def MPC_func_random(x, mpc, params, solver_inst, rand_noise):
        ...
        # bounds
        X_lower_bound = -5 * np.array([1e2, 1e2, 1, 1])#np.concatenate(-1e1 *5 * np.ones((mpc.ns-2) * (mpc.horizon)),-5 * np.ones((mpc.ns-2) * (mpc.horizon)))
        X_upper_bound = 5 * np.array([1e2, 1e2, 1, 1])#np.concatenate(1e1 *5 * np.ones((mpc.ns-2) * (mpc.horizon)),5 * np.ones((mpc.ns-2) * (mpc.horizon)))
...
def generate_experiment_notes(experiment_folder, params, params_innit, episode_duration, num_episodes, seed, alpha, sampling_time, gamma, decay_rate, decay_at_end, noise_scalingfactor, noise_variance, stage_cost_sum_before, stage_cost_sum_after):
...
experiment_folder_name = "v7_trial_8"
...
generate_experiment_notes(experiment_folder_name, params, params_original, episode_duration, num_episodes, seed, alpha, dt, gamma, decay_rate, decay_at_end, noise_scalingfactor, noise_variance, stage_cost_sum_before, stage_cost_sum_after)
