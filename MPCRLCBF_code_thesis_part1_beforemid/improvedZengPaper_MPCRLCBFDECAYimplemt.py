...
class MPC:
    ...
    def __init__(self, P=None):
        ...
        self.omega_sym = cs.MX.sym("w_k", self.horizon)
        self.omega_sing = cs.MX.sym("w_1")
    ...
    def cbf_func(self):
        cbf = self.dcbf(self.h_func, self.x_sym, self.u_sym, self.dynamics_f,
                        [lambda y: self.omega_sing * y])
        return cs.Function('cbff', [self.A_sym, self.B_sym, self.x_sym, self.u_sym, self.omega_sing],
    ...
    def cbf_const(self):
        ...
        for k in range(self.horizon):
            cbf_const_list.append(
                cbf_func(self.A_sym, self.B_sym, self.X_sym[:, k], self.U_sym[:, k], self.omega_sym[k])
            )
    ...
    def objective_method(self):
        stage_cost = sum(
            (self.X_sym[:, k].T @ self.Q @ self.X_sym[:, k] +
             self.U_sym[:, k].T @ self.R @ self.U_sym[:, k] + self.Pw_sym*((self.omega_sym[k] - self.omega0_sym)**2))
            for k in range(self.horizon)
        )
...
def MPC_func(x, mpc, Pw, omega0):
    ...
    lbx = np.concatenate([np.array(x).flatten(), mpc.X_lower_bound, U_lower_bound, np.ones(mpc.horizon)* np.array([1e-6])])
    ubx = np.concatenate([np.array(x).flatten(), mpc.X_upper_bound, U_upper_bound, np.ones(mpc.horizon)* np.array([1])])
...
def run_simulation_with_P_and_Pw(P_list, Pw_list, env_class):
    ...
    for P in P_list:
        for pw_idx, Pw in enumerate(Pw_list):
            ...
            print(f"Stage cost for P diag {np.diag(P)} and Pw {float(Pw)} is {np.sum(stage_cost_lst)} units")
...
P_list = [
    ...
    np.diag([84.4716, 96.0675, 2.87574, 11.7209]),
