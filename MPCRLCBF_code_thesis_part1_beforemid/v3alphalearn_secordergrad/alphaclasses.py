...
class RLclass:
        ...
        def parameter_updates(self, params, A_update_avg, B_update_avg):
            ...
            # lbx = cs.vertcat(-0.1*np.abs(theta_vector_num))
            # ubx = cs.vertcat(0.1*np.abs(theta_vector_num))
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, replay_buffer, episode_updatefreq, experiment_folder):
            ...
            for i in range(1,episode_duration*num_episodes):
                ...
                # alpha = self.fwd_func(x, params["nn_params"])

                # norm= self.mpc.nn.normalization_z(x)

                # print(f"norm: {norm}")

                # alpha = cs.DM(alpha)

                # print(f"alpha_1: {alpha}")
