...
class MPC:
    ...
    def MPC_solver(self):
        ...
        nlp = {
            ...
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.omega_sym),
            "f": self.objective,
...
def MPC_func(x, mpc, omega):
    ...
    solution = solver_inst(
        p=cs.vertcat(cs.reshape(A, -1, 1), cs.reshape(B, -1, 1), omega),
        ubx=ubx,
        lbx=lbx,
        ubg=ubg,
        lbg=lbg
...
def run_simulation_with_P_and_omega(P_list, omega_list, env_class):
    env_instance = env_class(sampling_time=0.2)
    # Define distinct marker styles for each omega value
    marker_list = ['o', 's', '^']
    
    for P in P_list:
        for om_idx, omega in enumerate(omega_list):
            state, _ = env_instance.reset(seed=42, options={})
            states = [state[:2]]
            mpc = MPC(P=P)  # Terminal cost matrix changes per simulation

            for _ in range(200):
                action, _ = MPC_func(state, mpc, omega)
            ...
            plt.plot(
                states[:, 0], states[:, 1],
                linestyle='-', marker=marker_list[om_idx],
                label=f"P: {np.diag(P)}, ω: {float(omega)}"
            )
            trajectory_length = calculate_trajectory_length(states)
            print(f"Trajectory length for P diag {np.diag(P)} and ω {float(omega)} is {trajectory_length:.2f} units")
    ...
    plt.xlabel("$x$ (m)")
    plt.ylabel("$y$ (m)")
    plt.title("Trajectories for varying terminal cost matrices P and ω")
    plt.legend()
    plt.axis("equal")
    plt.grid()
    plt.show()

# Define lists for P and omega values.
P_list = [
...
run_simulation_with_P_and_omega(P_list, omega_list, env)