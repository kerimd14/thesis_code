...
class MPC:
    ...
    def __init__(self, dt):
        ...
        self.weight_cbf = cs.MX.sym("w_pen", 1, self.horizon)
    ...
    def MPC_solver(self):
        ...
        nlp = {
            ...
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.Pw_sym, self.omega0_sym, self.weight_cbf),
            "f": self.objective, 
    ...
    def MPC_solver_rand(self):
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, S_flat, self.omega_sym),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.Pw_sym, self.omega0_sym, rand_noise, self.weight_cbf),
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        theta_vector = cs.vertcat(self.P_diag, self.Pw_sym, self.omega0_sym, self.weight_cbf)
        ...
        qlagrange_fn = cs.Function(
            ...
            [
                ...
                lagrange_mult_g_sym, X_flat, U_flat, S_flat, self.omega_sym, self.weight_cbf
            ],
            [qlagrange_sens],
            [
                'A_sym', 'B_sym', 'b_sym', 'Q_sym', 'R_sym',
                'P_sym', 'Pw_sym', 'omega0_sym', 'lagrange_mult_x_lb_sym', 
                'lagrange_mult_x_ub_sym', 'lagrange_mult_g_sym', 'X', 'U', 'S', 'omega_sym', 'w_pen'
            ],
            ['qlagrange_sens']
        )
...
class RLclass:
        ...
        def __init__(self, params_innit, seed, alpha, sampling_time, gamma, decay_rate, noise_scalingfactor, noise_variance):
            ...
            #ADAM
            self.exp_avg = np.zeros(7)
            self.exp_avg_sq = np.zeros(7)
            self.adam_iter = 1

            # RMSprop
            self.square_avg = np.zeros(7)
            self.grad_avg = np.zeros(7)
            self.momenum_buffer = np.zeros(7)
        ...
        def V_MPC(self, params, x):
            ...
            solution = self.solver_inst(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat, params["Pw"], params["omega0"], params["w_pen"]),
                ubx=ubx,  
                lbx=lbx,
                ubg=ubg,
                lbg=lbg
        ...
        def V_MPC_rand(self, params, x, rand):
            ...
            solution = self.solver_inst_random(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat, params["Pw"], params["omega0"], rand , params["w_pen"]),
                ubx=ubx,  
                lbx=lbx,
                ubg=ubg,
                lbg=lbg
        ...
        def stage_cost(self, action, x, S, omega):
            ...
            return (
                state.T @ Qstage @ state
                + action.T @ Rstage @ action + 1e4 * penalty
            )
        ...
        def parameter_updates(self, params, B_update_avg):
            ...
            theta_vector_num = cs.vertcat(P_diag, params["Pw"], params["omega0"], params["w_pen"])
            ...
            alpha_vec = cs.vertcat(self.alpha*np.ones(theta_vector_num.shape[0]-3), self.alpha,self.alpha*0.5e-1, self.alpha)
            ...
            # self.adam_iter += 1 
            dtheta, self.square_avg, self.grad_avg, self.momenum_buffer = self.RMSprop(B_update_avg, self.square_avg, self.alpha, 0.7, 1e-6, True, self.grad_avg, 0, self.momenum_buffer)
            ...
            solution = self.qp_solver(
                    ...
                    lbg=cs.vertcat(np.zeros(theta_vector_num.shape[0]-2), np.array([1e-6]), np.array([0])),
                    ubg = cs.vertcat(np.inf*np.ones(theta_vector_num.shape[0]-2), np.array([1]), np.array([np.inf])),
                )
            ...
            params["w_pen"] = theta_vector_num[-1]
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, replay_buffer, episode_updatefreq, experiment_folder):
            ...
            params_history_w_pen = [self.params_innit["w_pen"]]

            

            #for the for loop
            params = self.params_innit.copy()
            ...
            for i in range(1,episode_duration*num_episodes):
                ...
                if (k == episode_duration):
                    ...
                    if (i-1) % (episode_duration*episode_updatefreq) == 0:
                        ...
                        params_history_w_pen.append(params["w_pen"])
            ...
            params_history_w_pen = np.asarray(params_history_w_pen)
            ...
            figw_pen = plt.figure(figsize=(10, 5))
            plt.plot(params_history_w_pen[:, 0], label="pen weight$")
            plt.title("Parameter: Penalty Weight CBF")
            plt.xlabel("Training Step")
            plt.ylabel("Value")
            plt.legend()
            plt.grid()
