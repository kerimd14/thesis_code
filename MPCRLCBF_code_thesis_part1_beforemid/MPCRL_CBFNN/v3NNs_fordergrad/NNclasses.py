...
class NN:
    ...
    def normalization_z(self, nn_input):
        ...
        pos = cs.DM([-2, -2.25])
        r = cs.DM(1.5)
        h = (nn_input[0] - pos[0])**2 + (nn_input[1] - pos[1])**2 - r**2

        # mu_h = 6.7885445905109325
        # sigma_h = 0.4382974988082045
        h_min = 0#-2.25
        h_max = 11.8125
...
class RLclass:
        ...
        def stage_cost(self, action, x):
            ...
            return (
                ...
                + action.T @ Rstage @ action #+ np.abs(min(hx, 0))
            )
        ...
        def evaluation_step(self, params, experiment_folder, episode_duration):
                ...
                figs = [
                                (figstates, f"states_MPCeval_{self.eval_count}_SC:{sum_stage_cost}.png"),
                                (figactions, f"actions_MPCeval_{self.eval_count}_SC:{sum_stage_cost}.png"),
                                (figstagecost, f"stagecost_MPCeval_{self.eval_count}_SC:{sum_stage_cost}.png"),
                                (figsvelocity, f"velocity_MPCeval_{self.eval_count}_SC:{sum_stage_cost}.png")
                ]
