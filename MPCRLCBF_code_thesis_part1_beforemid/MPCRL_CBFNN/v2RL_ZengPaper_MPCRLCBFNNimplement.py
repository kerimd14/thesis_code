...
class MPC:
    ...
    def MPC_solver(self):
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, S_flat, Sx_flat),
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        opt_solution = cs.vertcat(X_flat, U_flat, S_flat, Sx_flat)

      
        # X_con + U_con + S_con 
        lagrange_mult_x_lb_sym = cs.MX.sym("lagrange_mult_x_lb_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + 1 * (self.horizon) + self.ns * (self.horizon))
        lagrange_mult_x_ub_sym = cs.MX.sym("lagrange_mult_x_ub_sym", self.ns * (self.horizon+1) + self.na * (self.horizon) + 1 * (self.horizon) + self.ns * (self.horizon))
        lagrange_mult_g_sym = cs.MX.sym("lagrange_mult_g_sym", 1*self.ns*(self.horizon) + self.horizon + 2*self.ns*(self.horizon))

        # construct 
        X_lower_bound = -5 * np.array([1e2, 1e2, 1e2, 1e2])#-1e6 *5* np.ones(self.ns * (self.horizon))
        X_upper_bound = 5 * np.array([1e2, 1e2, 1e2, 1e2])#1e6 * 5* np.ones(self.ns * (self.horizon)) 

        U_lower_bound = -np.ones(self.na * (self.horizon-1))
        U_upper_bound = np.ones(self.na * (self.horizon-1))  

        # X_con + U_con + S_con + Sx_con
        lbx = cs.vertcat(self.X_sym[:,0], cs.DM(X_lower_bound), self.U_sym[:,0], cs.DM(U_lower_bound), np.zeros(self.horizon), np.zeros(self.horizon*self.ns)) 
        ubx = cs.vertcat(self.X_sym[:,0], cs.DM(X_upper_bound), self.U_sym[:,0], cs.DM(U_upper_bound), np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns))
        ...
        qlagrange_fn = cs.Function(
            ...
            [
                ...
                lagrange_mult_g_sym, X_flat, U_flat, S_flat, Sx_flat, self.nn.get_flat_parameters()
            ],
            [qlagrange_sens],
            [
                'A_sym', 'B_sym', 'b_sym', 'Q_sym', 'R_sym','V_sym', 'P_sym', 'lagrange_mult_x_lb_sym', 
                'lagrange_mult_x_ub_sym', 'lagrange_mult_g_sym', 'X', 'U', 'S', 'Sx', 'inputs_NN'
            ],
            ['qlagrange_sens']
        )
...
class RLclass:
        ...
        def __init__(self, params_innit, seed, alpha, sampling_time, gamma, decay_rate, layers_list, noise_scalingfactor, noise_variance):
            ...
            self.X_lower_bound = -5 * np.array([1e2, 1e2, 1e2, 1e2])#-1e6 * 5 * np.ones(self.ns * (self.horizon))
            self.X_upper_bound = 5 * np.array([1e2, 1e2, 1e2, 1e2])#1e6 * 5 * np.ones(self.ns  * (self.horizon))
        ...
        def V_MPC(self, params, x):
            ...
            lbx = np.concatenate([np.array(x).flatten(), self.X_lower_bound, U_lower_bound,  np.zeros(self.horizon), np.zeros(self.horizon*self.ns)])  
            ubx = np.concatenate([np.array(x).flatten(), self.X_upper_bound, U_upper_bound,  np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns)])
        ...
        def Q_MPC(self, params, action, x):
            ...
            lbx = np.concatenate([np.asarray(x).flatten(), self.X_lower_bound, np.asarray(action).flatten(), U_lower_bound,  np.zeros(self.horizon), np.zeros(self.horizon*self.ns)])  
            ubx = np.concatenate([np.asarray(x).flatten(), self.X_upper_bound, np.asarray(action).flatten(), U_upper_bound, np.inf*np.ones(self.horizon), np.inf*np.ones(self.horizon* self.ns)])
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, experiment_folder):
            ...
            for i in range(1,episode_duration*num_episodes):
                ...
                qlagrange_numeric_jacob=  self.qlagrange_fn_jacob(
                    ...
                    X=X, U=U, S=S, Sx = Sx, inputs_NN=params["nn_params"]
                )['qlagrange_sens']
...
def MPC_func(x, mpc, params, solver_inst):
        ...
        # bounds
        X_lower_bound = -5 * np.array([1e2, 1e2, 1e2, 1e2])#-1e6 * 5 * np.ones(mpc.ns * (mpc.horizon))
        X_upper_bound = 5 * np.array([1e2, 1e2, 1e2, 1e2])#1e6 * 5 * np.ones(mpc.ns  * (mpc.horizon))
        ...
        lbx = np.concatenate([np.array(x).flatten(), X_lower_bound, U_lower_bound, np.zeros(mpc.horizon), np.zeros(mpc.horizon*mpc.ns)])  
        ubx = np.concatenate([np.array(x).flatten(), X_upper_bound, U_upper_bound, np.inf*np.ones(mpc.horizon), np.inf*np.ones(mpc.horizon* mpc.ns)])
        ...
        alpha = fwd_func(input, params["nn_params"])
...
def run_simulation(params, env, experiment_folder, episode_duration, layers_list, after_updates):
    ...
    for i in range(episode_duration):
        action, _, alpha = MPC_func(state, mpc, params, solver_inst)
...
def generate_experiment_notes(experiment_folder, params, params_innit, episode_duration, num_episodes, seed, alpha, sampling_time, gamma, decay_rate, decay_at_end, 
                              noise_scalingfactor, noise_variance, stage_cost_sum_before, stage_cost_sum_after, layers_list):
...
experiment_folder_name = "NN_trial_2"
