...
obs_position = cs.DM([-5, -5.25])

obs_position_plt = (-5, -5.25)
...
class env(gym.Env):
    ...
    def __init__(self, sampling_time:float):
        ...
        self.observation_space = Box(-x_constraints, x_constraints, (self.ns,), np.float64)
    ...
    def reset(self, seed, options):
        ...
        self.x = np.asarray([-x_constraints, -x_constraints, 0, 0])
...
class MPC:
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        # construct 
        X_lower_bound = -x_constraints * np.array([1, 1, 1, 1])#-1e6 * x_constraints * np.ones(mpc.ns * (mpc.horizon))
        X_upper_bound = x_constraints * np.array([1, 1, 1, 1])#1e6 * x_constraints * np.ones(mpc.ns  * (mpc.horizon))
...
class RLclass:
        ...
        def __init__(self, params_innit, seed, alpha, sampling_time, gamma, decay_rate, layers_list, noise_scalingfactor, noise_variance, patience_threshold, lr_decay_factor):
            ...
            # bounds
            #state bounded between x_constraints and -x_constraints
            self.X_lower_bound = -x_constraints * np.array([1, 1, 1, 1])#-1e6 * x_constraints * np.ones(mpc.ns * (mpc.horizon))
            self.X_upper_bound = x_constraints * np.array([1, 1, 1, 1])#1e6 * x_constraints * np.ones(mpc.ns  * (mpc.horizon))
        ...
        def evaluation_step(self, S, params, experiment_folder, episode_duration):
                ...
                # Plot the obstacle
                circle = plt.Circle(obs_position_plt, 1.5, color="k", fill=False, linewidth=2)
                plt.gca().add_patch(circle)
                plt.xlim([-x_constraints, 0])
                plt.ylim([-x_constraints, 0])
