...
def run_simulation(params, env, experiment_folder, episode_duration, layers_list, after_updates):
    ...
    circle = plt.Circle(obs_position_plt, 1.5, color='k', fill=False, linewidth=2)
    plt.gca().add_patch(circle)
    plt.xlim([-x_constraints,0]); plt.ylim([-x_constraints,0])
    ...
    plt.xlabel('Iteration $k$', fontsize=20); plt.ylabel('$\gamma$ Value', fontsize=20)
    # plt.title('$\gamma$ Colored by Iteration'); 
