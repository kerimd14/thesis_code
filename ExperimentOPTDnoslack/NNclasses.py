...
class MPC:
    ...
    def dcbf(self,
        ...
        for alpha in alphas:
            ...
            phi = phi_next + omega*(- phi + alpha(phi))
        return phi
    ...
    def cbf_func(self):
        ...
        return cs.Function('cbff', [self.x_sym, self.u_sym, self.nn.get_flat_parameters(), self.omega_sym], [cbf], ['x','u', 'alpha params', 'omega'], ['cbff'])
    ...
    def cbf_const(self):
        ...
        for k in range(self.horizon):
            ...
            cbf_const_list.append(cbf_func(self.X_sym[:,k], self.U_sym[:,k], self.nn.get_flat_parameters(), self.omega_sym)) 
    ...
    def objective_method_noslack(self):
        ...
        self.objective_noslack = self.V_sym + terminal_cost + stage_cost + opt_decay
    ...
    def MPC_solver_rand(self):
        ...
        nlp = {
            "x": cs.vertcat(X_flat, U_flat, self.omega_sym),
            "p": cs.vertcat(A_sym_flat, B_sym_flat, self.b_sym, self.V_sym, self.P_diag, Q_sym_flat, R_sym_flat, self.nn.get_flat_parameters(),
                            self.Pw_sym, self.omega0_sym, rand_noise),
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        qlagrange_fn = cs.Function(
            ...
            [
                ...
                lagrange_mult_g_sym, X_flat, U_flat,  self.nn.get_flat_parameters(),
                self.Pw_sym, self.omega0_sym, self.omega_sym
            ],
            [qlagrange_sens],
            [
                'A_sym', 'B_sym', 'b_sym', 'Q_sym', 'R_sym','P_sym', 'lagrange_mult_x_lb_sym', 
                'lagrange_mult_x_ub_sym', 'lagrange_mult_g_sym', 'X', 'U', 'inputs_NN', 'Pw_sym', 'omega0_sym', 'omega_sym'
            ],
            ['qlagrange_sens']
        )
...
class RLclass:
        ...
        def V_MPC_rand(self, params, x, rand):
            ...
            solution = self.solver_inst_random(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat, 
                                                              params["nn_params"], params["Pw"], params["omega0"], rand),
        ...
        def Q_MPC(self, params, action, x):
            ...
            solution = self.solver_inst(p = cs.vertcat(A_flat, B_flat, params["b"], params["V0"], P_diag, Q_flat, R_flat, 
                                                       params["nn_params"], params["Pw"], params["omega0"]),
                x0    = self.x_prev_QMPC,
        ...
        def rl_trainingloop(self, episode_duration, num_episodes, replay_buffer, episode_updatefreq, experiment_folder):
            ...
            for i in range(1,episode_duration*num_episodes):
                ...
                omega = solution[self.na * (self.horizon) + self.ns * (self.horizon+1): :self.na * (self.horizon) + self.ns * (self.horizon+1) +self.horizon]
                ...
                qlagrange_numeric_jacob=  self.qlagrange_fn_jacob(
                    ...
                    X=X, U=U, inputs_NN=params["nn_params"],
                    Pw_sym=params["Pw"],
